...
def initial_params(energy_mix, t_end, electricity_share_end):
    ...
    solar =  {'name': 'solar', 'cost_init': 10, 'power_init': 10,
            'tau_life': 10, 'td0': 10,
            'tau_exp': 4, 'fit_factor': 1e5, 'cap_factor': 0.1,
            'intermittent': True}
    ...
    wind = {'name': 'wind', 'cost_init': 1, 'power_init': 1,
            'tau_life': 1, 'td0': 1,
            'tau_exp': 4, 'fit_factor': 1e5, 'cap_factor': 0.01,
            'intermittent': True}
    ...
    nuclear = {'name': 'nuclear', 'cost_init': 100, 'power_init': 100,
            'tau_life': 100, 'td0': 100,
            'tau_exp': 4, 'fit_factor': 1e5, 'cap_factor': 1,
            'intermittent': False}
    ...
    storage = {'name': 'storage', 'cost_init': 1000, 'power_init': 0,
            'tau_exp': 4, 'tau_life': 1000, 'td0': 1000,'cap_factor': 1000,
            'fit_factor': 1e5, 'intermittent': False}
    storage['power_current'] = storage['power_init']
    storage['p_sat'] = sum([i['p_sat']/12/storage['efficiency'] for i in
...
energy_mix = {'solar': 0.25 ,'wind': 0.25, 'nuclear': 0.5}
t_end = 2050
max_budget = 1e6
electricity_share_end = 0.8
visualization = 1
...
def solver(parameters, energy_mix, t_end, max_budget, electricity_share_end, visualization):
    ...
    for year in range(t_end - t_init + 1):
        ...
        if visualization == 1:
            co2_yearly_matrix[year+1] = elec_non_renew*world['non_renew_co2']
            co2_total_matrix[year+1] = co2_total
    ...
    elec_renew = sum([i['power_current'] for i in list_of_params if i['name']!='storage'])
    percentage_renewables = elec_renew/(world['energy_demand_total']*world['electricity_share'][-1])*100 
    storage_current = sum([i['power_current']*i['efficiency'] for i in list_of_params if i['name']=='storage'])
    elec_int = sum([i['power_current'] for i in list_of_params if i['intermittent']==True])
    percentage_storage = storage_current/(elec_int/12)
